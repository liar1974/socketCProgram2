...
#define PHASE3LISTENER "3763"//UDP user2 static port

#define MAXDATASIZE 100 //max number of bytes in buffer we can get
#define MAXBUFLEN 100  // max number of bytes in buffer we can tet
...
//user2 listen the UDP port with supernode and get message from supernode. The UDP connection code is from Beej's Guide Book
int phase3listener()
{   //The following connection setup code comes from Beej's book
	...
	{   //If user get the finish signal, it will jump out the loop
	   ...
	   else
	   {
	      ...
	      printf("Phase 3: <User2> received the message %s",showSecondMessage);
	   }
	...
	close(sockfd);
    printf("End of Phase 3 for <User2>\n");
	return 0;
}
...
{   //The following connection setup code comes from Beej's book
	...
	int numbytes;
	int len;
	char getInfo[100];//Get the message from UserText2.txt
	struct hostent *host;
    ...
    if((fp = fopen("UserText2.txt", "r")) == NULL){
		printf("Can not open UserText2.txt\n");
		exit(1);
	}
	...
	printf("End of Phase 1 for <User2>.\n");
	printf("Phase 3: <User2> has static UDP port 3763 IP address %s.\n",inet_ntoa(*((struct in_addr*)host->h_addr_list[0])));
	//The following code will get message and send it to supernode

	//Send signal to notice that it is user2 to send the message to the supernode
    if ((numbytes = sendto(sockfd, "ab", 2, 0, p->ai_addr, p->ai_addrlen)) == -1) {
	...
	while(fgets(getInfo, 100, fp))
	{
			...
			printf("Phase 3: <User2> is sending the message %s on UDP dynamic port number %d\n",getInfo,ntohs(sin.sin_port));
	}

   //send signal to notice that user2 finish the process of sending message
...
{    //The following connection setup code comes from Beej's book
    ...
    FILE *fp;
     ...
     //Open UserPass2.txt
    if((fp = fopen("UserPass2.txt", "r")) == NULL){
		printf("Can not open UserPass2.txt\n");
		exit(1);
	}
   
	//Get each line of password and username in UserPass2.txt
